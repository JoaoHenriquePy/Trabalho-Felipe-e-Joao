...
def cadastrar_animal():
    ...
    if not tipo:
        print("Erro: Tipo não pode ficar em branco!")
        return

    identificacao = input("Identificação (brinco/número): ").strip()
    if not identificacao:
        print("Erro: Identificação não pode ficar em branco!")
        return
...
def registrar_vacina():
    ...
    if not vacina:
        print("Erro: Nome da vacina não pode ficar em branco!")
        return
...
def dashboard():
    ...
    tabela.add_row("Total Litros de Leite", str(total_leite))

    console.print(tabela)

    est_table = Table(title="📦 ESTOQUE")
    est_table.add_column("Produto")
    est_table.add_column("Quantidade")
    est_table.add_column("Preço")
    for item in estoque:
        est_table.add_row(item["nome"], str(item["quantidade"]), f"R$ {item['preco']:.2f}")
...
def consultar_movimentacoes():
    ...
    for mov in historico_movimentacao:
        print(f"{mov['data']} | {mov['acao'].upper()} | {mov['item']} | Qtd: {mov['qtd']}")


def consultar_clima():
    try:
        r = requests.get("https://wttr.in/Sousa?format=3", timeout=5)
        print("\n🌤️ Clima em Sousa:", r.text.strip())
    except:
...
def adicionar_desejo(usuario):
    ...
    if not produto:
        print("Erro: Produto não pode ficar em branco!")
        return
    lista_desejos.append({"usuario": usuario, "produto": produto})
    print("Desejo adicionado à lista!")