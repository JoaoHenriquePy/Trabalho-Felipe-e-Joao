    ...
    if op == 1:
        ...
        while i < len(usuarios):
            ...
            if nome_usuario == usuario_digitado and senha_usuario == senha:
                ...
                encontrado = True
                print(f"\n[SUCESSO] Bem-vindo, {usuario_logado}! Perfil: {tipo_logado}")
                break
            i = i + 1
       
        if not encontrado:
            print("\n[ERRO] Senha ou nome de usuário inválido!")
        else:
            continua_adm = "sim"
            while tipo_logado == "ADM" and continua_adm == "sim":
                print("\n--------- PAINEL DO ADMINISTRADOR ---------")
                print(" Opções: 1-Cadastrar | 2-Buscar | 3-Atualizar | 4-Remover")
                print("          5-Relatório R4 | 6-Produção R3 | 7-Sair do Painel")
                ...
                if proxima in ["cadastrar", "1"]:
                    ...
                    while tipo not in ["bovino", "caprino", "ovino", "suino"]:
                        print("[ERRO] Tipo inválido! Escolha entre Bovino, Caprino, Ovino ou Suíno.")
                        tipo = input("Tipo de animal: ").lower()
                        
                    identificacao = input("Identificação (Ex: Brinco, chip ou ID): ")
                    status = input("Status do animal (Ex: Venda, Engorda, Tratamento): ")
                    
                    animal = [tipo.capitalize(), identificacao, status]
                    rebanho.append(animal)
                    print(f"[SUCESSO] Animal {identificacao} cadastrado!")
                    ...
                    while i < len(rebanho):
                        if rebanho[i][1].lower() == buscar:
                            print(f"\n[ENCONTRADO] Tipo: {rebanho[i][0]} | ID: {rebanho[i][1]} | Status: {rebanho[i][2]}")
                            encontrado_busca = True
                        i = i + 1
                    if not encontrado_busca:
                        print("[AVISO] Animal não encontrado.")
                    ...
                    while i < len(rebanho):
                        if rebanho[i][1].lower() == buscar:
                            ...
                            print("[SUCESSO] Status updated!")
                            encontrado_atualizar = True
                            break  
                        i = i + 1  
                        
                    if not encontrado_atualizar:
                        print("[AVISO] Nenhum animal foi encontrado com essa identificação.")
                    ...
                    while i < len(rebanho):
                        if rebanho[i][1].lower() == remover:
                            del rebanho[i]
                            print("[SUCESSO] Animal removido do sistema!")
                            removido = True
                            break
                        i = i + 1
                    if not removido:
                    ...
                    print("\n=== RELATÓRIO DE REBANHO E VENDAS - R4 ===")
                    ...
                    while ponteiro < len(estoque):
                        if estoque[ponteiro][0].lower() == nome_prod_atualizar.lower():
                            ...
                            print("[SUCESSO] Estoque updated!")
                            achou_produto = True
                            break
                        ponteiro += 1
                    
                    if not achou_produto:
                        print("[ERRO] Produto não cadastrado no estoque inicial.")

                elif proxima in ["sair do painel", "7"]:
                    continua_adm = "nao"
                    print("Saindo do painel administrativo...")

            continia_cliente = "sim"
            while tipo_logado == "CLIENTE" and continia_cliente == "sim":
                print("\n--------- MENU CLIENTE ---------")
                print("1. Ver estoque e comprar")
                print("2. Histórico de compras - R7")
                ...
                if escolha == "1":
                    ...
                    if 0 <= num < len(estoque):
                        ...
                        qtd_desejada = float(input(f"Quantos(as) '{nome_prod}' você deseja? "))
                        ...
                        if qtd_desejada <= qtd_disponivel:
                            ...
                            if confirma == "s":
                                ...
                                print("[SUCESSO] Compra realizada com sucesso!")
                        else:
                            print("[ERRO] Quantidade insuficiente em estoque!")
                    else:
                        print("[ERRO] Opção de produto inválida.")
                    ...
                    while p < len(historico_compras):
                        ...
                        p = p + 1
                    print("[SUCESSO] Retirada agendada para os pedidos pendentes!")

                elif escolha == "4":
                    continia_cliente = "nao"
